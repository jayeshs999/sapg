...
from isaacgymenvs.pbt.launcher.checkpoint_utils import generate_experiments_from_checkpoint


def add_os_parallelism_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser.add_argument("--num_gpus", default=torch.cuda.device_count(), type=int, help="How many local GPUs to use")
    parser.add_argument("--max_parallel", default=None, type=int, help="Maximum simultaneous experiments")
    parser.add_argument(
        "--experiments_per_gpu",
        default=-1,
        type=int,
...
def run(run_description, args):
    max_parallel = args.max_parallel or 100
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES"):
        ...
        print("Overiding num_gpus with CUDA_VISIBLE_DEVICES", gpu_list)
        args.experiments_per_gpu = (args.num_policies + len(gpu_list) - 1) // len(gpu_list)
        args.num_gpus = len(gpu_list)
    else:
        print("CUDA_VISIBLE_DEVICES is not set, using all available GPUs")
        gpu_list = list(range(args.num_gpus))
    
    processes = []
    processes_per_gpu = {g: [] for g in gpu_list}

    if args.checkpoint_dir is None:
        experiments = run_description.generate_experiments(args.train_dir)
    else:
        experiments = generate_experiments_from_checkpoint(args.checkpoint_dir, args.multi_gpu)
    ...
    while len(processes) > 0 or next_experiment is not None:
        while can_squeeze_another_process() and next_experiment is not None:
            ...
            # Set multi GPU args
            if args.multi_gpu:
                envvars["LOCAL_RANK"] = f"{len(processes)}"
                envvars["RANK"] = f"{len(processes)}"
                envvars["WORLD_SIZE"] = f"{args.num_policies}"
